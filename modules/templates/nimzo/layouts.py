        ...
        if item.enabled and item.visible:
            ...
            if item.parent is not None:
                ...
                if item.parent.parent is None:
                    ...
                    toplevel = True
                    if item.opts.right:
                        classes.append("right-sided")
                else:
                    toplevel = False

                if item.components:
                    classes.append("has-dropdown not-click")
                    classes.append("ui floating dropdown item")
                    _class = " ".join(classes)
                    # Menu item with Dropdown
                    if item.get_first(enabled=True):
                        _href = item.url()
                        return DIV(SPAN(item.label),
                                   DIV(items,
                                       _class="menu"
                                       ),
                                   _href=_href,
                ...
                inner = []
                inner.append(A(IMG(_src="/eden/static/img/S3menu_logo.png"),
                               _href=URL(c="default", f="index"),
                               _class="S3MenuLogo item"
                               ))

                for item in items:
                    if "right-sided" in item["_class"]:
                        item.remove_class("right-sided")
                        right.append(item)
                    else:
                        left.append(item)

                inner.extend(left)
                inner.append(DIV(right,
                                 _class="right menu"))

                return DIV(
                    inner,
                    _class="ui inverted blue menu",
                )
            ...
            if item.parent is not None:
                if item.enabled and item.authorized:
                    if item.components:
                        ...
                        return CAT(A(item.label,
                                     _href=item.url(),
                                     _id=item.attr._id,
                                     _class="%s header item" % _class
                                     ),
                                   DIV(*section,
                                       _class="menu"))
